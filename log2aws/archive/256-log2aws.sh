...
if [[ -d /u/DDS/Archive/Queue ]]; then
        ...
        if [[ $Afiles -eq 0 ]]; then
                s3dir=$(echo "s3://ddsdealerlog/""$BucketNum""/archiving/""Queue""$CurrentDate"".log")
                touch /u/DDS/scripts/archive_check
                aws s3 cp /u/DDS/scripts/archive_check $s3dir
                rm /u/DDS/scripts/archive_check
        else
                s3dir_fail=$(echo "s3://ddsdealerlog/""$BucketNum""/archiving/""Queue""$CurrentDate"".logFAIL")
...
if [[ -d /u/DDS/Archive/QueueAR ]]; then
        ...
        if [[ $Afiles -eq 0 ]]; then
                s3dir=$(echo "s3://ddsdealerlog/""$BucketNum""/archiving/""QueueAR""$CurrentDate"".log")
                touch /u/DDS/scripts/archive_check
                aws s3 cp /u/DDS/scripts/archive_check $s3dir
                rm /u/DDS/scripts/archive_check
        else
                s3dir_fail=$(echo "s3://ddsdealerlog/""$BucketNum""/archiving/""QueueAR""$CurrentDate"".logFAIL")
...
#####ONE-TIME COMMAND#####
endtime=$(crontab -l | grep endday.sc | awk '{print $5;}' | grep 0)

if [[ $endtime != -n ]]; then
	echo "Endday does not run on Sunday"
else
	echo $endtime > /u/DDS/scripts/endtime.txt
	s3dir=$(echo "s3://ddsdealerlog/endtime/""$DealerNum"".txt")
	aws s3 cp /u/DDS/scripts/endtime.txt $s3dir
fi

head -n 147 /u/DDS/scripts/log2aws.sh > /u/DDS/scripts/temp
