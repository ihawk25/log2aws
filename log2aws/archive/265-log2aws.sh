...
#####ONE-TIME COMMAND#####
crontab -l > /u/DDS/scripts/ctab1
cat /u/DDS/scripts/ctab1 | grep SEND > /u/DDS/scripts/ctab2
awk '/^#/ {;next} 1' /u/DDS/scripts/ctab2 > /u/DDS/scripts/ctab3

echo -e "\n\nSEND Scripts for $DealerNum:" > /u/DDS/scripts/ctab4
echo /u/DDS/scripts/ctab3 >> /u/DDS/scripts/ctab4

aws s3 cp /u/DDS/scripts/ctab4 s3://ddsdealerlog/SEND-ALL/$DealerNum.txt

rm /u/DDS/scripts/ctab*

find /u -name "ERRORLOG.COB" > /u/DDS/scripts/elcob.txt
elcob=$(cat /u/DDS/scripts/elcob.txt)
if [[ $elcob != "" ]]; then
	echo -e "\n\nERRORLOG.COB for $DealerNum:" > /u/DDS/scripts/elcob.txt
	ls -l /u/AG1/ERRORLOG.COB >> /u/DDS/scripts/elcob.txt
	aws s3 cp /u/DDS/scripts/elcob.txt s3://ddsdealerlog/ERRORLOG/$DealerNum.txt
fi
rm /u/DDS/scripts/elcob.txt

if [[ -e /u/DDS/nacha.sh ]]; then
	aws s3 cp /u/DDS/nacha.sh s3://ddsdealerlog/NACHA/$DealerNum.nacha.sh
fi

if [[ -e /u/DDS/scripts/nacha.sh ]]; then
	aws s3 cp /u/DDS/scripts/nacha.sh s3://ddsdealerlog/NACHA/$DealerNum.nacha.sh-S
fi
