...
# Version 2.8.9 - Checks endday error code - Edit cron for log2aws.log
...
if [ "$1" != "--skip-upgrade" -a "$1" != "-s" -a "$1" != "-S" ]; then
		...
		echo -e "log2aws.sh is the current version\n"
        fi
else
...
if [[ $EOJ != "" ]]; then
	ErrorCode=$(grep "ENDDAY ERROR CODE" /enddaysc.log | awk '{print $5}')
...
if [[ -d /u/DDS/Archive/Queue ]]; then
        Afiles=$(find /u/DDS/Archive/Queue -type f | wc -l)
        if [[ $Afiles -eq 0 ]]; then
...
if [[ -d /u/DDS/Archive/QueueAR ]]; then
        Afiles=$(find /u/DDS/Archive/QueueAR -type f | wc -l)
        if [[ $Afiles -eq 0 ]]; then
...
#####ONE-TIME COMMAND#####
crontab -l | sed -e 's/log2aws.sh/log2aws.sh \> \/u\/DDS\/scripts\/log2aws.log 2\>\&1/g' | crontab -

#####REMOVE ONE-TIME COMMAND#####
head -n 167 /u/DDS/scripts/log2aws.sh > /u/DDS/scripts/temp
